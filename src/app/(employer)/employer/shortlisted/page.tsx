...
export const metadata: Metadata = {
  title: 'Shortlisted Candidates — Placely',
  description: 'Review shortlisted and offered candidates across your SIWES listings.',
};

export default async function EmployerShortlistedPage() {
  ...
  for (const listing of listings) {
    ...
    for (const item of applicants) {
      if (item.application.status === 'SHORTLISTED' || item.application.status === 'OFFERED' || item.application.status === 'ACCEPTED') {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-xl font-bold tracking-tight text-foreground">Shortlisted Candidates</h1>
        <p className="text-sm text-muted-foreground mt-1">
          {shortlisted.length} candidate{shortlisted.length !== 1 ? 's' : ''} in your pipeline
        </p>
      </div>
      ...
      {sorted.length === 0 ? (
        <div className="rounded-card border border-dashed border-slate-200 py-12 px-6 bg-white text-center space-y-3">
          ...
          <div className="space-y-1">
            <h3 className="text-sm font-bold text-slate-900">No shortlisted candidates yet</h3>
            <p className="text-xs text-slate-500 max-w-sm mx-auto">
              Review applicants from your listings and shortlist promising candidates here.
            </p>
          </div>
          <Link
          ...
          >
            Review Listings <ArrowRight className="w-3.5 h-3.5" />
          </Link>
        </div>
      ) : (
