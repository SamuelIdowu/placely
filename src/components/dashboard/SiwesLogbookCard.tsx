...
import { BookOpen, CheckCircle2, Clock, Plus, ArrowRight, ShieldCheck } from "lucide-react";
...
export function SiwesLogbookCard({
  ...
  const percentComplete = Math.min(100, Math.round((currentWeek / totalWeeks) * 100));
  ...
  return (
    ...
    >
      ...
      <div
        className="absolute -right-12 -top-12 w-48 h-48 rounded-full opacity-10 pointer-events-none blur-2xl bg-brand-indigo"
      />
      ...
      <div>
        ...
        <div className="flex items-center justify-between gap-3 mb-3">
          ...
          <span
            className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-stat-emerald/15 text-stat-emerald border border-stat-emerald/30"
          >
        ...
        <div
          className="rounded-xl p-3 flex items-center justify-between gap-3 text-xs mb-3.5 bg-white/5 border border-white/10"
        >
          <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-[11px] text-emerald-400">
            {isSupervisorSigned ? "Week 3 Signed Off" : "Sign-off Pending"}
          </span>
        </div>
      </div>
